...
\title{Series VI – Article VI.1: $K$‑Theory Spectrum and Transfer Operator}
...
The construction of $T_p$ is formalised in \texttt{KummerVandiver/TransferOperator.lean}. The code defines the group ring, the Adams operation, and proves self‑adjointness and integrality of eigenvalues.
...
lemma T_self_adjoint (k : ℕ) : (T p k).adjoint = T p k := by
  sorry  -- proved using orthonormal basis of characters

lemma eigenvalues_in_Z : ∀ λ ∈ spectrum (T p k), λ ∈ ℤ := by
  sorry  -- proved via character theory
\end{lstlisting}
