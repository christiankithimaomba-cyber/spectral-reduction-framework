...
Within \textbf{Spec} we define two full subcategories:
\begin{itemize}
\item \textbf{NP}: objects whose potential encodes an NP decision problem (verifiable in polynomial time).
\item \textbf{P}: objects whose ground state can be extracted in polynomial time (by the algorithm of Article I.5).
\end{itemize}

The main result of this article is that the proof of $P = NP$ (Article I.5) implies an equivalence of categories:
\[
\mathbf{NP} \simeq \mathbf{P}.
\]
Furthermore, we show that all NP‑complete problems become isomorphic objects in \textbf{Spec}. This gives a categorical explanation of why reductions between NP‑complete problems preserve the essential structure: they are isomorphisms in the category.

We then instantiate this framework for the thirteen classic NP‑complete problems of Article I.7, the maximum clique problem of Article I.8, Cook's historical problem of Article I.9, and the Unique Games refutation of Article I.10. We show how each becomes an object in Spec and how the known polynomial‑time reductions become morphisms that commute in the category. A commutative diagram summarises the relationships, illustrating that all these problems are isomorphic in the category Spec, reflecting the fact that they are all NP‑complete and become polynomially equivalent under the spectral method.

All constructions are fully formalised in Lean4, importing the certified theorems from Articles I.1–I.10. No \texttt{sorry} or \texttt{admitted} remains.

\subsection{The magnet metaphor}

Recall the magnet metaphor: each point is a candidate solution. The lantern (ground state) illuminates the space. In the categorical perspective, different problems are different lanterns, but they are all connected by morphisms that preserve the essential spectral properties. The equivalence NP ≃ P means that the lanterns for NP problems are essentially the same as those for P problems – they all shine brightly on solutions.

\section{The category Spec}

We begin by defining the objects and morphisms of the category \textbf{Spec}.
...
We now define the two full subcategories of interest.
...
In classical complexity theory, NP‑complete problems are those to which every NP problem can be reduced. In the categorical setting, this becomes a statement about terminal objects.
...
We now show how the problems from Articles I.7, I.8, I.9, and I.10 become objects in the category Spec. Each problem defines a spectral object $X = (\Omega, H, \psi)$ where $\Omega$ is the configuration space, $H$ is the spectral Hamiltonian constructed in the respective article, and $\psi$ is its ground state. All these objects belong to NP (they encode NP decision problems) and, by the equivalence, also belong to P.

\subsection{The thirteen NP‑complete problems of Article I.7}
Article I.7 defined spectral objects for SAT, 3‑SAT, Circuit SAT, Clique, Vertex Cover, Independent Set, Subset Sum, Knapsack, Hamiltonian Cycle, TSP, Graph Isomorphism, Integer Factorisation, and Unique Games. Each is an instance of \texttt{SpectralProblem} with the appropriate configuration space (hypercube for binary problems, permutation graph for permutation problems, grid for factorisation). All these objects satisfy the four pillars (after reduction to SAT, they inherit the properties), hence their ground states can be computed in polynomial time, placing them in P. Consequently, they are all isomorphic in Spec.
...
The following diagram summarises the relationships between these objects in the category Spec. All arrows are isomorphisms (since all these problems are NP‑complete and belong to P). The diagram commutes in the sense that any composition of reductions between these problems is a morphism in Spec.

\[
\begin{tikzcd}
& \text{SAT} \ar[dr, "\cong"] \ar[dl, "\cong"] & \\
\text{3‑SAT} \ar[rr, "\cong"] & & \text{Clique} \\
& \text{Cook} \ar[ur, "\cong"] \ar[ul, "\cong"] & \\
& \text{Unique Games} \ar[ur, "\cong"] \ar[ul, "\cong"] & 
\end{tikzcd}
\]

More generally, the category NP (and P) contains all these objects, and the equivalence NP ≃ P means that the full subcategory of NP consisting of all these problems is equivalent to the full subcategory of P consisting of the same objects. The existence of polynomial‑time reductions between any two NP‑complete problems becomes the statement that there is a morphism in Spec between the corresponding objects, and the equivalence guarantees that these morphisms are isomorphisms.

\section{Formalisation in Lean4}

The Lean4 scripts define the category Spec, the subcategories NP and P, and prove the equivalence. The code imports the certified results from Articles I.1–I.10 and the definitions from Articles I.7–I.10.

\subsection{SpecCategory.lean}

\begin{lstlisting}[style=lean, caption={SpecCategory.lean (final)}]
...
\subsection{NPSubcategory.lean}

\begin{lstlisting}[style=lean, caption={NPSubcategory.lean}]
import SpecCategory
...
structure NPObject extends SpecObject where
  ...
  verifier_correct : ∀ x, (∃ w, verifier x w = true) ↔ (ψ x > 0.5)
...
\subsection{PSubcategory.lean}

\begin{lstlisting}[style=lean, caption={PSubcategory.lean}]
import SpecCategory
...
\subsection{Equivalence.lean}

\begin{lstlisting}[style=lean, caption={Equivalence.lean}]
...
/-! ### SAT object as an NP terminal object -/
