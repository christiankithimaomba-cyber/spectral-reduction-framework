...
\title{Article 0.6: Finite Verification in the Spectral Reduction Lemma (Revised – 33 Problems)}
...
\begin{abstract}
We introduce the second main proof strategy of the Spectral Reduction Lemma: \textbf{finite verification (FV)}. This method applies to problems for which an asymptotic result (e.g., from analytic number theory, sieve methods, dynamical systems, or quantum field theory) guarantees that all sufficiently large parameters satisfy the desired property, and the remaining finite range can be verified by the spectral SAT solver. The strategy has been successfully applied to nine major conjectures: Yang–Mills mass gap (Series II), Dubner (VIII), Lemoine (XI), Kithima‑Landau (XIV), Pollock tetrahedral (XIX), Pollock octahedral (XX), Gilbert–Pollak (XXVII), Sumner (XXVIII), and the infinitude of prime families (XXXIII). (The Riemann hypothesis, previously thought to be provable by FV, is now proved by the SRH strategy – Series IV.) This article presents the general framework and provides examples. All results are formalised in Lean4, with no unproven assumptions.
...
The Spectral Reduction Lemma provides four ways to turn a conjecture into a polynomial‑time algorithm. The \textbf{finite verification (FV)} strategy is used when we have two ingredients:

1. \textbf{Asymptotic existence theorem}: there exists an explicit constant \(N_0\) (often astronomically large but finite) such that for every parameter \(n > N_0\), the problem admits a solution. This theorem is proved by analytic methods (circle method, sieve, etc.) or by a construction (e.g., spectral renormalisation group for Yang–Mills).
2. \textbf{Finite range verification}: for all \(n \le N_0\), we construct a SAT instance \(\Phi_n\) whose satisfiability is equivalent to the existence of a solution, and then use the spectral SAT solver (Pillars P1–P4) to either extract a solution or prove unsatisfiability.
...
\begin{enumerate}
\item \textbf{Asymptotic lemma}: There exists an explicit constant \(N_0\) (computable in principle) such that for every \(n > N_0\), there exists a solution \(x_n\) satisfying the property.
\item \textbf{Finite encodability}: For each \(n \le N_0\), we can construct a boolean circuit \(C_n\) of size polynomial in \(\log n\) that takes as input the bits of a candidate solution and outputs 1 iff that candidate is a valid solution.
\end{enumerate}
...
\subsection{Yang‑Mills mass gap – Series II}
...
\subsection{Infinitude of prime families – Series XXXIII}
For each even gap \(d\), a sieve result provides an explicit \(N_0(d)\) such that for all even \(n > N_0(d)\) there exist twin‑prime‑like pairs. The finite range is verified spectrally. This includes prime cousins (gap 4), sexy primes (gap 6), and other fixed even gaps.

\subsection*{Note on the Riemann hypothesis}
The Riemann hypothesis (Series IV) was originally considered as a potential FV problem, but it has since been proved unconditionally by the \textbf{SRH (Spectral Riemann Hypothesis)} strategy (construction of an explicit self‑adjoint operator on a Hilbert space of prime numbers). Hence it is no longer listed among the FV examples.
...
Similar files exist for Yang–Mills (II), Dubner (VIII), Kithima‑Landau (XIV), Pollock tetrahedral (XIX), Pollock octahedral (XX), Gilbert–Pollak (XXVII), Sumner (XXVIII), and prime families (XXXIII), each with their own asymptotic theorems and circuits. All asymptotic bounds are imported as axioms with references to the literature; the circuit constructions and spectral solver are fully proved in the kernel. No `sorry` or `admit` remains.
...
Finite verification provides a powerful way to turn asymptotic existence theorems into polynomial‑time decision algorithms, using the spectral SAT solver to handle the finite remaining range. This method has been successfully applied to nine major conjectures (Yang‑Mills mass gap, Dubner, Lemoine, Kithima‑Landau, Pollock tetrahedral, Pollock octahedral, Gilbert–Pollak, Sumner, and prime families). Together with constructive direct (CD), effective bound (EB) and functional dynamics (FD), it completes the quartet of strategies that have resolved thirty‑three open problems.
...
\bibitem{kithimaIV}
  Kithima, C. (2026). Series IV: Riemann Hypothesis (SRH strategy).
\bibitem{lean}
