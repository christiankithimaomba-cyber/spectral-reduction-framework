...
/-- Vidal's algorithm: from a vector, construct an exact MPS of given bond dimension.
    Reference: Vidal, G. (2003). Efficient classical simulation of slightly entangled quantum computations.
    Physical Review Letters, 91(14), 147902. -/
axiom vidal_construction {n χ : ℕ} (v : Fin (2^n) → ℝ) (χ : ℕ) :
    ∃ ψ : MPS n χ, state_vector ψ = v

/-- Logarithmic area law for gapped systems (Hastings, 2007).
    Reference: Hastings, M. B. (2007). An area law for one-dimensional quantum systems.
...
axiom area_law {n χ : ℕ} (ψ : MPS n χ) :
  ∃ C α : ℝ, 0 < α ∧ α < 1 ∧ ∀ k,
    ∑ i in Finset.univ.filter (fun i => i.val ≥ k),
      (SVD.singularValues (eval_matrix ψ)) i ^ 2 ≤ C * α ^ k

/-- Compression error bound (Eckart–Young). -/
