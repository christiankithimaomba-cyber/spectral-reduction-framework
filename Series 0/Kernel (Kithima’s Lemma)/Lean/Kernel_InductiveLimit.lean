...
/-- Construction of an inductive limit of a sequence of Hilbert spaces with isometric embeddings.
    Reference: Kato, T. (1966). Perturbation Theory for Linear Operators. Springer. -/
axiom inductive_limit_exists {H : ℕ → Type*} [∀ k, InnerProductSpace ℝ (H k)]
    (i : ∀ k, H k →ₗᵢ[ℝ] H (k+1)) : Type*

/-- Inductive limit operator. -/
...
/-- Strong resolvent convergence for the inductive limit (Kato). -/
