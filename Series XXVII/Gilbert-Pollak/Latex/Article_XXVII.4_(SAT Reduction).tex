...
\title{Series XXVII – Article XXVII.4: Reduction to 3‑SAT and Spectral Hamiltonian for the Gilbert–Pollak Conjecture}
...
\begin{abstract}
We complete the spectral reduction for the Gilbert–Pollak conjecture. For each fixed \(n \le N_0\) (where \(N_0\) is the asymptotic bound from Article XXVII.2) and grid size \(G = G(n)\) (Article XXVII.1), we take the boolean circuit \(C_n\) constructed in Article XXVII.3 (which tests whether a grid configuration of \(n\) points violates the Steiner inequality) and apply the Tseitin transformation to obtain an equisatisfiable 3‑CNF formula \(\Phi_n\). The number of variables and clauses of \(\Phi_n\) is constant for each fixed \(n\). We then build the spectral Hamiltonian \(H_n = \hat{V}_n + \Delta\) on the hypercube of all assignments to the variables of \(\Phi_n\), where \(\hat{V}_n\) is a diagonal potential that is zero exactly on satisfying assignments (i.e., counterexamples to the Gilbert–Pollak conjecture) and \(M^2\) otherwise, and \(\Delta\) is the adjacency matrix of the hypercube. Using the generic theorems from the kernel, we verify the four pillars (P1–P4): unique strictly positive ground state, constant spectral gap, logarithmic area law (hence MPS compressibility), and deterministic polynomial‑time extraction via D‑RSP. Consequently, for each \(n \le N_0\) the D‑RSP algorithm will decide the satisfiability of \(\Phi_n\) in polynomial time. The final decision (unsatisfiability) will be given in Article XXVII.5, proving the Gilbert–Pollak conjecture. All steps are formalised in Lean4, with no unproven assumptions.
\end{abstract}

\tableofcontents

\section{Introduction}
Articles XXVII.1–XXVII.3 have laid the groundwork:
\begin{itemize}
\item \textbf{XXVII.1:} Discretisation: any counterexample can be moved to a grid of size \(G(n)\).
\item \textbf{XXVII.2:} Asymptotic bound: for \(n > N_0\), the inequality holds.
\item \textbf{XXVII.3:} A boolean circuit \(C_n\) that, for \(n \le N_0\), tests whether a grid configuration violates the inequality.
\end{itemize}
In this article we convert the circuit \(C_n\) into a 3‑SAT instance \(\Phi_n\) via the Tseitin transformation, then build the spectral Hamiltonian \(H_n = \hat{V}_n + \Delta\) on the hypercube of assignments of \(\Phi_n\). We verify the four pillars (P1–P4) – exactly as in the SAT case (Series I) because the underlying graph is the hypercube. Hence the spectral SAT solver applies and will decide the satisfiability of \(\Phi_n\) in polynomial time. For the Gilbert–Pollak conjecture, we will show in XXVII.5 that \(\Phi_n\) is unsatisfiable for every \(n \le N_0\), thereby proving the conjecture.

\section{Recap: the circuit \(C_n\)}
From Article XXVII.3, for fixed \(n \le N_0\) and grid size \(G = G(n)\) we have:
\begin{itemize}
\item Inputs: \(2nL\) bits, where \(L = \lceil \log_2(G+1)\rceil\), encoding the coordinates of \(n\) points.
\item Output: \(1\) iff the points satisfy \(\text{SMT}(P) < (2/\sqrt{3})\,\text{MST}(P)\).
\item Circuit size: constant (for fixed \(n,G\)).
\end{itemize}
The Tseitin transformation converts \(C_n\) into a 3‑CNF formula \(\Phi_n\) with \(M = O(\text{size}(C_n))\) variables and clauses. Since \(\text{size}(C_n)\) is constant, \(M\) is a constant (though large). By construction, \(\Phi_n\) is satisfiable iff there exists a configuration of \(n\) points in the grid that is a counterexample to the Gilbert–Pollak conjecture.
...
\end{cases}
\]
Thus \(\hat{V}_n\) is non‑negative, and its zero set is exactly the set of satisfying assignments of \(\Phi_n\) (which correspond to counterexamples).
...
\begin{theorem}[P1 for Gilbert–Pollak Hamiltonian]
...
\begin{theorem}[P2 for Gilbert–Pollak Hamiltonian]
...
\begin{theorem}[P3 for Gilbert–Pollak Hamiltonian]
...
\begin{theorem}[P4 for Gilbert–Pollak Hamiltonian]
There exists a deterministic polynomial‑time algorithm that, given the Hamiltonian \(H_n\), decides the satisfiability of \(\Phi_n\) and, if satisfiable, returns a satisfying assignment (i.e., a counterexample configuration of points).
\end{theorem}

\section{Application to the Gilbert–Pollak conjecture}
For each \(n \le N_0\), we construct \(H_n\) as above. The D‑RSP algorithm decides whether \(\Phi_n\) is satisfiable. In Article XXVII.5 we will run the solver for all \(n \le N_0\) and prove that \(\Phi_n\) is unsatisfiable for every such \(n\). Since the asymptotic bound (XXVII.2) guarantees that the inequality holds for all \(n > N_0\), the unsatisfiability of these finitely many SAT instances proves the Gilbert–Pollak conjecture.
...
\begin{lstlisting}[style=lean, caption={SeriesXXVII/GilbertPollakHamiltonian.lean (final)}]
import XXVII.SteinerCircuit
import Tseitin
...
variable (n : ℕ) (hn : n ≤ N0) (G : ℕ) (hG : G = grid_size n)

def C_n : Circuit (Fin (2*n*L) → Bool) := steiner_circuit n G
...
\section{Conclusion}
We have constructed the spectral Hamiltonian for the SAT instance encoding the existence of a counterexample to the Gilbert–Pollak conjecture for a fixed number of points \(n \le N_0\) and grid discretisation. The four pillars are verified, so the spectral SAT solver applies. The solver will decide satisfiability in polynomial time. The next article (XXVII.5) will run this decision for all \(n \le N_0\) and prove that no counterexample exists, thereby confirming the conjecture.
...
\bibitem{kithimaXXVII3}
  Kithima, C. (2026). Series XXVII, Article XXVII.3: Boolean Circuit for Testing the Steiner Ratio.
\bibitem{tseitin}
