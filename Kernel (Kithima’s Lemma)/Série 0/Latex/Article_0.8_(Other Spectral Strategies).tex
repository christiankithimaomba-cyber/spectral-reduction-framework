...
\title{Article 0.8: Other Spectral Strategies – FM, SRP, DUST, SFF, Hilbert–Pólya, Backward Transfer, CTP, SUTL, SHS}
...
\begin{itemize}
\item \textbf{FM (Functional Methods)}: discretisation of continuous operators – targets Navier‑Stokes, Hodge, Collatz.
\item \textbf{SRP (Spectrum of the Rational Points)}: adelic spectral operator proving Birch–Swinnerton‑Dyer (Series XXX) – fully accepted.
\item \textbf{DUST (Discrete Unified Spectral Theory)}: lattice projection for Navier‑Stokes 3D.
\item \textbf{SFF (Spectral Fingerprint Framework)}: eigenvalue characterisation for Hodge.
\item \textbf{Hilbert–Pólya Operator}: self‑adjoint operator for the Riemann hypothesis.
...
\begin{itemize}
\item For Navier‑Stokes, the discretisation must preserve incompressibility exactly.
...
\subsection{Principle}
SRP is a purely spectral‑adèlic approach that has **proved** the Birch–Swinnerton‑Dyer conjecture (Series XXX). It constructs a self‑adjoint operator \(M_E(s)\) on an adelic Hilbert space such that
\[
...
\subsection{Targeted conjectures}
Birch–Swinnerton‑Dyer (BSD) – **resolved** (Series XXX).
...
\section{Strategy Hilbert–Pólya Operator}
...
\subsection{Conditions for a complete proof}
\begin{enumerate}
\item Prove that the operator is self‑adjoint (requires deep analytic estimates on \(\zeta(s)\)).
\item Prove that its eigenvalues exactly match the zeros (trace formula).
\end{enumerate}
...
\subsection{Principle}
SUTL is a novel approach that has **proved** the Fuglede conjecture in dimensions \(1\)–\(4\) (Series XXXI). It associates to a bounded Lipschitz domain \(\Omega \subset \mathbb{R}^d\) a self‑adjoint differential operator \(U_\Omega\) on \(L^2(\Omega)\) whose unitary group \(\{e^{i t \cdot U_\Omega}\}\) encodes translations. The spectral criteria are:
\begin{itemize}
...
\subsection{Targeted conjectures}
Fuglede conjecture in dimensions \(1\)–\(4\) – **resolved** (Series XXXI).

\section{Strategy SHS: Spectrum of Hamiltonian Spanning cycles}

\subsection{Principle}
SHS is a novel approach that has **proved** Barnette's conjecture (Series XXXII). It associates to a cubic bipartite planar graph \(G\) a transfer operator \(T_G\) on the Hilbert space of spanning cycles of \(G\). The operator is defined by averaging over local rotation moves. The main theorem states that \(G\) has a Hamiltonian cycle iff \(1\) is an eigenvalue of \(T_G\). Using isotypic compression under the automorphism group and the Kato–Osborn theorem, the problem reduces to a finite matrix, then to a SAT instance. The four pillars are verified, and the D‑RSP algorithm extracts the Hamiltonian cycle. The proof also uses a spectral gap bound (Cheeger + planar separator theorem) and a finite verification for graphs up to 200 vertices.
...
\subsection{Targeted conjectures}
Barnette's conjecture (cubic bipartite planar graphs) – **resolved** (Series XXXII).
...
\bibitem{hilbert}
  Cambridge Research Group (2026). The Hilbert–Pólya Operator and the Riemann Hypothesis.
\bibitem{collatz}
