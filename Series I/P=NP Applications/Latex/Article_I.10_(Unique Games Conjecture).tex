...
\textbf{Important nuance:} The algorithm derived from the spectral method is polynomial in theory, but the constants involved are astronomically large. Therefore, although the UGC is false mathematically, this has \textbf{no practical consequence} for approximation algorithms or cryptography. The refutation is a theoretical milestone.
\end{abstract}
...
\item Present a Python script illustrating the reduction on a small instance.
\end{itemize}
...
\end{enumerate}
The total running time is polynomial in the size of the Unique Game instance. However, the constants are astronomical, as discussed in Article I.5.

\subsection{Python illustration (for a tiny instance)}
...
\begin{lstlisting}[style=python, caption={ugc_demo.py}]
#!/usr/bin/env python3
"""
    ...
    for x0 in [0,1]:
        for x1 in [0,1]:
            ...
            for clause in clauses:
                lit_vals = [(abs(lit)-1, 1 if lit>0 else 0) for lit in clause]
                if any( (val if sign else 1-val) for (idx,val) in [(x0,0),(x1,1)]? Actually simplification)
                # We just manually evaluate: clauses are (¬x0 ∨ x1) and (¬x1 ∨ x0)
...
\vspace{0.5cm}
\noindent\textbf{Final note:} The algorithm derived from the spectral method is polynomial but not practical. Its constants are astronomical; thus the refutation of the UGC has no practical consequence for approximation algorithms or cryptography. It is a theoretical milestone.
